...
/// @version  v2.0.0
/// @date     2019-03-13
/// @brief    THIS FILE PROVIDES ALL THE SLEEP EXAMPLE.
////////////////////////////////////////////////////////////////////////////////
/// @attention
///
...
// Step 2:  Create File Device   ---------------------->>>>>
    HANDLE hSLEEP = CreateFile(emIP_POWER);
    if (hSLEEP == NULL)     while(1);

// Step 3:  Assignment DCB structure    --------------->>>>>
    tAPP_POWER_DCB dcb = {
        .hSub           = emFILE_SLEEP,                                         // emFILE_SLEEP
        .sync           = emTYPE_ASync                                          // emTYPE_Sync or emTYPE_ASync
    };
    if (!OpenFile(hSLEEP, (void*)&dcb))     while(1);
    ...
    while(1) {
        if (user_Key2() || cntKey2 > 1000) {
            ...
            if (cntKey2 > 1000) {                                               // Key2 has been pressed for more than 1 seconds
                ...
                if (!user_Key2()) {                                             // If released Key2, close all Led, set RTC AlarmCounter and entering Sleep mode
                    cntKey2 = 0;
                    vdLED = 0x00;
                    SysDisplay((u8*)&vdLED);

                    /* ====  Setting Sleep  ==== */
                    WriteFile(hSLEEP, emFILE_SLEEP,  NULL, NULL);
